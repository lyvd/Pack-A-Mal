...
def module_paths_to_import(package):
    ...
    paths = list_python_files(package)
    return paths


def import_package(package):
    """Import phase for analyzing the package."""
    for p in module_paths_to_import(package.local_path):
        # import_module(p)
        import_single_module(p)
...
def import_single_module(import_path):
    ...
    module_name = os.path.basename(module_dir) + '.' + os.path.basename(import_path).removesuffix(PY_EXTENSION)
...
PHASES = {
    'all': [import_package, execute_module],
    # 'install': [install],
    'import': [import_package],
    'execute': [execute_package],
}

def extract_tar(tar_gz_file, output_dir):
    try:
...
def main() -> int:
    ...
    if local_path is not None:
        ...
        destionation_dir = os.path.join(current_dir, package_name)
        extract_tar(local_path, destionation_dir)
        local_path = destionation_dir
