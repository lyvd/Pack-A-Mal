...
type Result struct {
	...
	syscallSequence  []string //  Store the system call sequence
}
...
func (r *Result) recordCommand(cmd []string) {
	// Convert the command slice to a single string for easier comparison
	cmdStr := fmt.Sprintf("%s", cmd)

	// Check if the command contains the filtered strings
	if strings.Contains(cmdStr, "/usr/local/bin/analyze_wolfi_apk.py") ||
		strings.Contains(cmdStr, "sleep 30m") ||
		strings.Contains(cmdStr, "apk add --allow-untrusted") {
		return
	}

	// Record the command if it doesn't contain the filtered strings
	if _, exists := r.commands[cmdStr]; !exists {
		r.commands[cmdStr] = &CommandInfo{
			Command: cmd,
		}
	}
}
...
func (r *Result) parseEnterSyscall(syscall, args string, logger *slog.Logger) error {
	r.syscallSequence = append(r.syscallSequence, fmt.Sprintf("Enter: %s(%s)", syscall, args))
	switch syscall {
	case "write":
...
func (r *Result) parseExitSyscall(syscall, args string, logger *slog.Logger) error {
	r.syscallSequence = append(r.syscallSequence, fmt.Sprintf("Exit: %s(%s)", syscall, args))
	switch syscall {
	case "creat":
	...
	case "execve":
		...
		cmd, _, err := parseCmdAndEnv(match[1])
		if err != nil {
...
func Parse(ctx context.Context, r io.Reader, debugLogger *slog.Logger) (*Result, error) {
	...
	// for _, syscall := range result.syscallSequence {
	// 	slog.InfoContext(ctx, "System call", "syscall", syscall)
	// }
...
// parse systemcall sequence
func (r *Result) ParseSyscallSequence() []string {
	return r.syscallSequence
}
